    ...
    def forward(self, inputs):
        ...
        if not self.first_layer:
            if len(compound_prompts_deeper) > 0:
                ...
                if not self.text_layer: # since text and visual side transformer is different 
                    ...
                    if not (counter > len(compound_prompts_deeper) - 1):
                        ...
                        # Create/configure learnable tokens of this layer
                        textual_context = compound_prompts_deeper[counter] # [n_ctx,dim], learnable text token in each deep layer
                        textual_context = textual_context.expand(x.shape[1], -1, -1).permute(1, 0, 2).half() #[n_ctx,N,dim]
                        # Add the learnable tokens of this layer with the input, replaced by previous
                        # layer learnable tokens
                        x = torch.cat([prefix, textual_context, suffix], dim=0)
                        # Once done, update the counter, so that the next time, it does not use same learnable tokens
